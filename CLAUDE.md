...
30. **발송 버튼**: 빨간색(#CC2222) + 수리X 트리거 추가
31. **팝��� 드래그 이동**: 문자함+고객이력 팝업 헤더 드래그로 자유 이동
32. **확인 컬럼 phone 매칭**: as_record_id→phone 기준 + ilike 견적 + 즉��� 업데이트
33. **AS상태+입금 뱃지 강렬 색상**: 진한 배경+흰색 글씨 (6+6)
34. **검색 드롭다운 필터링**: 거래처 클릭 → 테이블 필터링 (팝업 안 열림) + 즉시 닫힘
35. **뱃지펼침 position:fixed**: overflow:hidden 잘림 방지 + 스크롤 닫힘
36. **테이블 뱃지 크기 통일**: min-width별 통일 + 중앙정렬
37. **테이블 폰트/크기/정렬 통일**: Pretendard, 텍스트13px, 날짜12px, 금액파란볼드, center
